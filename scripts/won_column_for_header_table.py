...
for file_name in header_files:
    ...
    # Create the 'won' column based on the conditions
    df['won'] = df.apply(
        lambda row: 'team_a' if row['score_a'] > row['score_b'] else ('team_b' if row['score_a'] < row['score_b'] else 'draw'),
        axis=1
    )

    # Save the modified DataFrame to a new file with "_with_won" suffix
    output_path = folder_path + file_name.replace('.csv', '_with_won.csv')
    df.to_csv(output_path, index=False)

    print(f"Modified file saved to {output_path} with the new 'won' column.")
