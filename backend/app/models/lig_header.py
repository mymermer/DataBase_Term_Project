...
class Lig_HeaderDAO():
    ...
    def create_lig_header(db: db, header: Lig_Header) -> None:
        """
        Insert a new LIG_HEADER record with only provided columns.

        Args:
            db: Database connection.
            header: A `Lig_Header` object or dictionary containing column-value pairs for the record.
        """
        try:
            ...
            query = f"INSERT INTO LIG_HEADER ({columns}) VALUES ({placeholders})"

            # Execute query with provided values
            cursor.execute(query, list(header.values()))
            connection.commit()

            print(f"Successfully created LIG_HEADER with provided data.")
    ...
    @staticmethod
    def get_paginated_lig_header(db: db, offset: int = 0, limit: int = 25, columns: list = None, filters: dict = None, sort_by: str = None, order: str = 'asc') -> list:
        try:
            connection = db.get_connection()
            ...
            query = f"""
                SELECT {selected_columns} FROM LIG_HEADER
                {where_clause}
                {order_clause}
                LIMIT %s OFFSET %s
            """
            ...
            # Map fetched rows to Lig_Header objects or dicts
            if columns:
    ...
    @staticmethod
    def get_total_lig_header(db: db, filters: dict = None) -> int:
        try:
            connection = db.get_connection()
    ...
    @staticmethod
    def get_paginated_lig_header_with_like(
        db: db,
        like_pattern: str,
        offset: int = 0,
        limit: int = 25,
    ...
    def get_distinct_games_with_like(
        ...
        try:
            ...
            query = f"""
                SELECT DISTINCT {selected_columns} FROM LIG_HEADER
                {where_clause}
            """
