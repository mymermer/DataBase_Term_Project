...
class Cup_PlayersDAO():
    ...
    def create_cup_players(db: db, player: Cup_Player) -> None:
        try:
            ...
            query = f"INSERT INTO CUP_PLAYERS ({columns}) VALUES ({placeholders})"
    ...
    @staticmethod
    def get_cup_player(db: db, season_player_id: str) -> Cup_Player: 
        try:
            connection = db.get_connection()
            query = """
    ...
    @staticmethod
    def get_all_cup_players(db: db) -> list:
        try:
            connection = db.get_connection()
            query = """
            ...
            if players is None:
                return None
            return [Cup_Player(*player) for player in players]
        except mysql.connector.Error as err:
            print(f"Error: {err}")
            connection.rollback()
        finally:
    ...
    @staticmethod
    def update_cup_player(db: db, player:Cup_Player) -> None:
        try:
            connection = db.get_connection()
            cursor = connection.cursor()
