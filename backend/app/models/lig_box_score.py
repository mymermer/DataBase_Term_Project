...
class LigBoxScoreDAO:
    ...
    def get_total_lig_box_scores(db: db, filters: dict = None) -> int:
        """
        Returns a COUNT of rows in LIG_BOX_SCORE,
        optionally with a set of 'column = value' filters.
        """
        try:
            connection = db.get_connection()
            cursor = connection.cursor()
            ...
            # If filters_raw was "game:OLYMPIACOS PIRAEUS BC,phase:Regular"
            # then filters = {"game": "OLYMPIACOS PIRAEUS BC", "phase": "Regular"}
            if filters:
                for column, value in filters.items():
                    where_clauses.append(f"{column} = %s")
                    params.append(value)
    ...
    def get_distinct_games_with_like(
        ...
        try:
            ...
            query = f"""
                SELECT DISTINCT {selected_columns} FROM LIG_BOX_SCORE
                {where_clause}
            """
