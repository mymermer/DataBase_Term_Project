...
def get_cup_header(game_id):
    # Fetch the game data
    header = Cup_HeaderDAO.get_cup_header(db, game_id)
    if not header:
    ...
    response = {
        ...
        "team_a": header.team_a,
        "team_b": header.team_b,
...
def get_paginated_header():
    try:
        ...
        cup_header = Cup_HeaderDAO.get_paginated_cup_header(
            db, 
            offset=offset, 
            limit=limit, 
            columns=columns, 
...
@cup_header_bp.route('/cup_header/with_year_like', methods=['GET'])
def get_paginated_headers_with_like():
    ...
    try:
        ...
        cup_headers = Cup_HeaderDAO.get_paginated_cup_header_with_like(
            db,
        ...
        if cup_headers is None:
            return jsonify([]), 200
        return jsonify(cup_headers), 200  # Already a list of dicts if columns are specified
    except ValueError:
        return jsonify({'error': 'Invalid offset, limit, columns, filters, sortBy, or order'}), 400
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
@cup_header_bp.route('/cup_header/year_distinct_games', methods=['GET'])
def get_distinct_header_games_with_like():
    ...
    try:
        ...
        query = """
            SELECT game_id, game FROM CUP_HEADER WHERE game_id LIKE %s
        """
