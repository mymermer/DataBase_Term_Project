...
export default function TeamsPage({ params }) {
  ...
  const handleAdd = async (newRowData) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:5000/api/v1/${tournament}_teams`,
        {
          method: "POST",
          headers: {
  ...
  const handleDelete = async (season_team_id) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:5000/api/v1/${tournament}_teams/${season_team_id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <StatPageTemplate league={league} stat="Teams">
      <div className={styles.PageContent}>
        <h2>
          {league.charAt(0).toUpperCase() + league.slice(1)} Teams Statistics
        </h2>
